    ...
    const fetchDomainAge = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:3000/api/domain-age/${domain}`
        );
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-6 text-white">
        <h2 className="font-bold text-xl font-mono">Domain Age Checker</h2>
        ...
        {loading ? (
          ...
          <dl className="font-mono">
            ...
            <dd className="font-normal">{result.registrationYear}</dd>
            ...
            <dd className="font-normal">{result.registrationDate}</dd>
          </dl>
        ) : (
          <p className="font-normal mt-4">Enter a domain to check its age.</p>
        )}
      </div>
    </div>
  );
